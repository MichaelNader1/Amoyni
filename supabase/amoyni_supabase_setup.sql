...
-- This file implements a simple CUSTOM auth model instead:
--   - profiles.password_hash and admin_users.password_hash store
--     bcrypt hashes (via pgcrypto).
--   - youth_login()/admin_login() verify credentials and return the
--     user/admin row as JSON. The client stores the returned id
--     locally (e.g. localStorage) as its "session".
...
--     and do their own authorization checks using the user id the
--     client passes in.
...
-- close_meeting
-- ---------------------------------------------------------------
create or replace function close_meeting(p_meeting_id uuid, p_admin_id uuid)
returns jsonb
language plpgsql
security definer
as $$
...
begin
  ...
  values (p_admin_id, 'close_meeting', 'meetings', p_meeting_id, 'إغلاق اجتماع',
...
begin
  ...
  v_diff_seconds := extract(epoch from (v_server_time - p_scan_started_at));
  if abs(v_diff_seconds) > 60 then
    raise exception 'SCAN_TIMEOUT';
  end if;

  if exists (select 1 from attendance_records where meeting_id = p_meeting_id and user_id = p_user_id) then
  ...
  -- Determine point rule based on scan start time
  select * into v_rule
  from meeting_point_rules
  where meeting_id = p_meeting_id
    and p_scan_started_at >= start_time
    and p_scan_started_at < end_time
  ...
  insert into attendance_records (
    ...
    p_meeting_id, p_user_id, p_scan_started_at, v_server_time,
  ...
  if v_points > 0 then
    insert into point_transactions (
      ...
      p_user_id, 'attendance', 'credit', v_points, v_balance_before, v_balance_after,
      'attendance_records', v_attendance_id, 'حضور اجتماع: ' || v_meeting.title
    );
  end if;

  perform recalculate_user_streak(p_user_id);
  ...
  if v_first_attendance and v_profile.referred_by_user_id is not null then
    ...
    if found and v_referral_settings.is_enabled then
      perform activate_referral_reward(p_user_id);
    end if;
  end if;
  ...
  return jsonb_build_object(
    ...
    'streak', (select current_streak from profiles where id = p_user_id),
...
create policy meetings_public_read on meetings for select
...
grant select on avatars, meetings, donation_campaigns,
...
-- perform their own authorization checks using the id the client
-- passes in, consistent with the custom-auth model documented at the
...
create or replace function create_meeting(
  p_admin_id uuid, p_title varchar, p_meeting_date date,
...
begin
  ...
  values (p_title, p_meeting_date, p_attendance_start, p_attendance_end,
    ...
    p_raffle_start, p_raffle_end, 'draft', p_admin_id)
  returning id into v_id;

  insert into audit_logs (admin_id, action, entity_type, entity_id, description, new_data)
  values (p_admin_id, 'create_meeting', 'meetings', v_id, 'إنشاء اجتماع: ' || p_title,
...
create or replace function add_point_rule(
  p_admin_id uuid, p_meeting_id uuid, p_start_time timestamptz,
...
create or replace function delete_point_rule(p_admin_id uuid, p_rule_id uuid)
returns jsonb
language plpgsql
security definer
as $$
...
create or replace function get_meeting_details(p_meeting_id uuid)
returns jsonb
language sql
security definer
as $$
...
create or replace function start_meeting(p_admin_id uuid, p_meeting_id uuid)
returns jsonb
language plpgsql
security definer
as $$
...
begin
  ...
  insert into audit_logs (admin_id, action, entity_type, entity_id, description)
  values (p_admin_id, 'start_meeting', 'meetings', p_meeting_id, 'بدء الاجتماع وتفعيل QR');
